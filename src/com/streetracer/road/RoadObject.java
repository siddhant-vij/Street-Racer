...
import com.streetracer.raceutils.GameObject;
import com.streetracer.raceutils.ShapeMatrix;
...
public class RoadObject extends GameObject {
  ...
  public boolean isCollisionWithDistance(RoadObject roadObject, int distance) {
    if ((x - distance > roadObject.x + roadObject.width)
        || (x + width + distance < roadObject.x)) {
      return false;
    }

    if ((y - distance > roadObject.y + roadObject.height)
        || (y + height + distance < roadObject.y)) {
      return false;
    }
